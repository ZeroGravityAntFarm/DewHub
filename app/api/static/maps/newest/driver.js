...
function loadNewestMaps(page = 1) {
    scroll(0, 0);
    const xhttp = new XMLHttpRequest();
    xhttp.open("GET", "/api_v2/maps/newest?page=" + page + "&size=21");
    xhttp.send();
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (let [i, object] of data["items"].entries()) {
                ...
                trHTML += '<a href="/api_v2/maps/' + object['mapName'] + '/file" class="btn btn-primary me-1">Map File</a>';
                trHTML += '<a href="/api_v2/maps/' + object['mapName'] + '/variant/file" class="btn btn-primary me-1">Variant File</a>';
                trHTML += '</div>';
            ...
            trHTML += generatePagination(pages, current, "loadNewestMaps");
