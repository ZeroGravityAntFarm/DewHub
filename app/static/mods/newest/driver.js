...
function loadNewest(page = 1) {
    ...
    xhttp.open("GET", "/api_v2/mods/newest?page=" + page + "&size=21");
    xhttp.send();
...
function searchMods(page = 1) {
    scroll(0, 0);
    var trHTML = '';
    const xhttp = new XMLHttpRequest();
    var searchQuery = document.getElementById('search_bar').value;
    console.log(searchQuery);
    xhttp.open("GET", "/api_v2/mods/search/" + searchQuery + "?page=" + page + "&size=21");
    xhttp.send();
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            const data = JSON.parse(this.responseText);
            ...
            for (let [i, object] of data["items"].entries()) {
                ...
                trHTML += '<a href="/api_v2/modview?modId=' + object['id'] + '"><img src="/mods/tb/' + object['id'] + '/0" class="card-img h-100" alt="..." onerror="this.onerror=null;this.src=\'/content/default/forge.jpg\';"></a>';
                trHTML += '</div>';
                trHTML += '<div class="col-md-8">';
                trHTML += '<div class="card-body">';
                trHTML += '<h3 class="card-title">' + object['modName'] + '</h3>';
                trHTML += '<h5 class="card-title">Author: ' + object['modAuthor'] + '</h5>';
                trHTML += '<h5 class="card-title">Type: ' + object['modTags'] + '</h5>';
                trHTML += '<p class="card-text">' + object['modDescription'] + '</p>';
